...
export class UploadFileCloud implements UploadFileGateway {
  ...
  async uploadFile(stream: Readable, filename: string, destinationPath: string): Promise<string> {
    try {
      const bucket = this.storage.bucket(this.bucketName)
      const file = bucket.file(destinationPath)

      logger.info(
        `Fazendo upload do arquivo ${filename} para ${this.bucketName}/${destinationPath}`,
      )
